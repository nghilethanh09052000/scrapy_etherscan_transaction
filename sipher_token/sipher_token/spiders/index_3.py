...
class Index3Spider(scrapy.Spider):
    ...
    def start_requests(self):
        data = pd.read_csv('ather_wallets_2.csv')

        dict_data = data.to_dict(orient='records')
        
        wallet_addresses = [ i.get('wallet_address') for i in dict_data ]

        for wallet_address in wallet_addresses:

            yield scrapy.Request(
                url      = f'https://etherscan.io/address/{wallet_address}#multichain-portfolio',
                method='GET',
                meta={
